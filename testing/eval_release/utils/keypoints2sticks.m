...
if joint_n == 14 % LSP and PARSE
  ...
  sticks(:,1:2,:)   = [(keypoints(:, 1, :) + keypoints(:, 4, :))/2, (keypoints(:, 7, :) + keypoints(:, 8, :))/2];         % Torso
  sticks(:,3:4,:)   = [keypoints(:, 1, :), keypoints(:, 2, :)];                                % Left Upper Arm
  sticks(:,5:6,:)   = [keypoints(:, 2, :), keypoints(:, 3, :)];                                % Left Lower Arm
  sticks(:,7:8,:)  = [keypoints(:, 4, :), keypoints(:, 5, :)];                                % Right Upper Arm
  sticks(:,9:10,:) = [keypoints(:, 5, :), keypoints(:, 6, :)];                                % Right Lower Arm
end
end